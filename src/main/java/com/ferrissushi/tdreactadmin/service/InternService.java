...
public class InternService {
    ...
    public List<Intern> getAll(String sort, String order, int start, int end,
                                Long idManager, String department, Boolean hasSalary,
                                String q) {
        Stream<Intern> stream = repo.findAll().stream();

        if (idManager != null)
            stream = stream.filter(i -> idManager.equals(i.getIdManager()));
        if (department != null)
            stream = stream.filter(i -> department.equals(i.getDepartment()));
        if (hasSalary != null)
            stream = stream.filter(i -> hasSalary.equals(i.getHasSalary()));
        if (q != null && !q.isBlank())
        ...
        Comparator<Intern> comp = switch (sort) {
