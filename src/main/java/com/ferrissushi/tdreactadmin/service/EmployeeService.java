...
public class EmployeeService {
  ...
  public List<Employee> getAll(
      String sort, String order, int start, int end, String department, Boolean active, String q) {
    Stream<Employee> stream = repo.findAll().stream();

    if (active != null) stream = stream.filter(e -> active.equals(e.getActive()));
    if (department != null) stream = stream.filter(e -> department.equals(e.getDepartment()));
    if (q != null && !q.isBlank())
      stream =
          stream.filter(
              e ->
